...
export default function UsuarioForm({
  ...
  const { register, control, handleSubmit } = useForm({
    defaultValues: {
      ...
      perfis: valoresIniciais.perfis?.map((p) => PERFIL_IDS[p.nome ?? p]).filter(Boolean) ?? [],
    },
  });
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="row g-3" noValidate>
      ...
      {perfis.length > 0 && (
        <div className="col-12">
          ...
          <Controller
            ...
            control={control}
            render={({ field }) => (
              <div className="d-flex flex-wrap gap-2" role="group" aria-label="Perfis do usuário">
                {perfis.map((perfil) => {
                  ...
                  return (
                    <button
                      ...
                      className={`btn btn-sm ${selecionado ? "btn-primary" : "btn-outline-secondary"}`}
                    >
                      {selecionado && <i className="bi bi-check me-1" aria-hidden="true" />}
                      {perfil.nome}
                    </button>
                  );
                })}
      ...
      <div className="col-12 d-flex gap-2 justify-content-end pt-2">
        <button type="button" className="btn btn-outline-secondary btn-sm" onClick={onCancelar} disabled={salvando}>
          Cancelar
        </button>
        <button type="submit" className="btn sge-btn-login btn-sm text-white" disabled={salvando}>
          {salvando ? (
